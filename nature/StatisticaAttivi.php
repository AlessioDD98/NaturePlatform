...
<html lang="en">
<title>
Statistica utenti più attivi
</title>
<head>
<meta charset="utf-8">
<link href="stilef.css" rel="stylesheet" type="text/css">
<br>
<h1 class="card row justify-content-center align-items-center">Statistica Utenti Più Attivi</h1>
</head>
<body>
<br>
...
<?php
  try {
   ...
   try{
   $sql="SELECT Nickname,count(*) as tot from Segnalazione group by Nickname order by tot";
   $res=$pdo->query($sql);
     while($row=$res->fetch()){
        echo('Nickname: '.$row['Nickname'].'<br> Totale: '.$row['tot'] ."<br>");
        }
      }catch(PDOException $e)	{
