...
<html lang="en">
<title>
Visualizza Segnalazioni Classificate Di Vegetali
</title>
<head>
<meta charset="utf-8">
<link href="stilef.css" rel="stylesheet" type="text/css">
<br>
<h1 class="card row justify-content-center align-items-center">Visualizza Segnalazioni Classificate Di Vegetali</h1>
</head>
<body>
<br>
...
<?php
  ...
  $sq="SELECT * FROM classificazionevegetale";
  $result=$pdo->query($sq);
  while($row=$result->fetch()){
    ...
    $res=$pdo->query($sql);
    $res0=$res->fetch();
    $sql1="SELECT NomeLatino FROM classificazionevegetale WHERE (IDSegnalazione='$ids')";
    $res=$pdo->query($sql1);
    $res1=$res->fetch();
    echo ("ID: ".$res0["ID"]."<br>");
