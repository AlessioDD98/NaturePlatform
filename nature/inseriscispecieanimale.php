...
<?php
  ...
  $numeromedioprole=$_POST["numprole"];
  ...
  if($classe!="" && $nomelatino!="" && $nomeitaliano!="" && $livvulnerabilita!="" && $nomehabitat!=""){
    try{
        $sql1="CALL InserisciSpecieAnimale('$classe','$nomelatino','$nomeitaliano','$annoclassificazione','$livvulnerabilita','$link','$peso','$altezza','$numeromedioprole','$nomehabitat','$nickname')";
  ...
  try {
     ...
     $doc = ['_id' => new MongoDB\BSON\ObjectID(),'data'=>date("Y-m-d h:i:sa"),'name' => $nickname, 'azione' => 'Inserimento Specie Animale', 'esito'=> $esito];
     $bulk->insert($doc);
