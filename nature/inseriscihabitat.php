...
if($res['Categoria']=="Amministratore"){
  if($nome!=""){
    try{
        $sql1="CALL InserisciHabitat('$nome','$descrizione','$nickname')";
        $result=$pdo->exec($sql1);
        echo("Inserimento avvenuto con successo!");
        $esito=true;
        include("mioprofilo.php");
      }catch(PDOException	$e)	{
    ...
    $doc = ['_id' => new MongoDB\BSON\ObjectID(),'data'=>date("Y-m-d h:i:sa"),'name' => $nickname, 'azione' => 'Inserimento Habitat', 'esito'=> $esito];
    $bulk->insert($doc);
